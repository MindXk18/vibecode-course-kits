...
2. Debug systematically. Use `debugger` subagent if needed.
...
- If stuck, reconsider the approach from a different angle
...
- Use `TaskUpdate`/`TaskList` for task sync-back and plan status updates
...
| 2 | `debugger` subagent, parallel `Explore` |
| 4 | `tester` subagent, parallel `Bash` verification |
| 5 | `code-reviewer` subagent |
| 6 | Native Task tools, `git-manager`, `docs-manager` subagents |
