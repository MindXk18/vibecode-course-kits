...
Before designing, extract brand colors/fonts/voice from the user's project (e.g. `docs/brand-guidelines.md`) and any design tokens (spacing, typography scale, color palette) already defined.

1. **Randomly invoke ONE of:** `/mx:ui-ux-pro-max` OR `/mx:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Use Chrome headless or Playwright/Puppeteer to capture exact-size screenshots.
...
Use Chrome MCP / `chrome-devtools-mcp` or Playwright to visually inspect each exported PNG:
