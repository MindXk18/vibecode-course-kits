...
4. **Design** — Randomly invoke `/mx:ui-ux-pro-max` OR `/mx:frontend-design`; HTML per idea × size
5. **Export** — Chrome headless or Playwright screenshot at exact px (2x deviceScaleFactor)
6. **Verify** — Use Chrome MCP / `chrome-devtools-mcp` or Playwright to visually inspect exported designs; fix layout/styling issues and re-export
