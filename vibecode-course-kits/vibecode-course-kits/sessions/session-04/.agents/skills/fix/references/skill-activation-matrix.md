...
|------------|------|--------|
| `Explore` (parallel) | Step 1 | Understand codebase context before diagnosing |
| Native Task tools (`TaskUpdate`/`TaskList`) | Step 6 | MANDATORY for sync-back and progress tracking, every fix |
...
|----------|------------------|
| Quick | `code-reviewer`, parallel `Bash` verification |
| Standard | Above + Tasks, `tester`, parallel `Explore` |
| Deep | All above + `researcher`, `planner` |
...
| Step 1: Scout | 2-3 parallel `Explore` → map files, deps, tests |
| Step 2: Diagnose | Capture pre-fix state → form hypotheses through structured reasoning → parallel `Explore` hypotheses → (if 2+ fail, reconsider approach) |
...
| Step 6: Finalize | Report → `TaskUpdate` → `docs-manager` → `git-manager` |
