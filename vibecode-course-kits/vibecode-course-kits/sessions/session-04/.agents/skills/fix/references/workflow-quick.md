...
Diagnose systematically through structured reasoning.
...
1. **Sync plan/task status (MANDATORY)** → if fix is part of a plan, update progress, hydrate Claude Tasks.
...
5. Log decisions in a concise summary.
...
|------|------------------|
| 1 | Direct file read (minimal scout) |
| 2 | Structured diagnosis reasoning |
| 3 | Parallel `Bash` for verification |
| 4 | `code-reviewer` subagent |
| 5 | Report |
| 6 | Sync plan/task status (MANDATORY), `docs-manager`, `git-manager` |
...
- Step 6 finalize (sync plan/task status, docs, commit) is MANDATORY for every fix — it is NOT optional
