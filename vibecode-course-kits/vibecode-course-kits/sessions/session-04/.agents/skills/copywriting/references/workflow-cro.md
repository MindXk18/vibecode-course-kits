...
1. **Screenshots provided** → Analyze directly for conversion issues
2. **Videos provided** → Analyze directly for bottleneck identification
3. **URL provided** → Use `web_fetch` tool to fetch and analyze current issues
4. **Scout codebase** → explore the codebase (e.g. parallel `Explore` subagents) to find relevant files
5. **Implement** → Use `fullstack-developer` agent to write enhanced copy into code files

## CRO Plan Creation Workflow

Use when creating a structured CRO optimization plan with CRO-specific requirements.

### Steps

1. **Analyze** → Gather issues from screenshots/videos/URLs using steps above
2. **Create plan directory** using naming pattern from `## Naming` section (hook-injected)
3. **Write `plan.md`** with required YAML frontmatter:
   ```yaml
   ---
   title: "{Brief title}"
   ...
   ---
   ```
4. Keep `plan.md` generic, under 80 lines, list each phase with status/progress and links
5. **Create phase files** (`phase-XX-phase-name.md`) with sections: Context links, Overview (date/priority/statuses), Key Insights, Requirements, Architecture, Related code files, Implementation Steps, Todo list, Success Criteria, Risk Assessment, Security Considerations, Next steps
6. Keep research markdown reports ≤150 lines with citations
7. **Wait** for user approval before implementing
