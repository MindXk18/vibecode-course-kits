...
2. Debug systematically — trace symptoms back to root cause, not guesswork.
...
- Use `TaskUpdate`/`TaskList` for task sync-back, plan status updates, and progress tracking
...
| 2 | parallel `Explore` (hypothesis testing) |
| 3 | `researcher` (runs parallel with steps 1+2) |
| 5 | `planner` |
| 7 | `tester`, parallel `Bash` verification |
| 8 | `code-reviewer` |
| 9 | Native Task tools, `docs-manager`, `git-manager` |
