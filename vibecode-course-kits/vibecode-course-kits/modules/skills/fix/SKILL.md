...
1. Launch 2-3 parallel `Explore` subagents
...
2. Debug systematically — trace root cause, not surface symptoms.
3. Form hypotheses through structured reasoning, NOT guessing.
4. Spawn parallel `Explore` subagents to test each hypothesis against codebase evidence.
5. If 2+ hypotheses fail, reconsider the approach or broaden investigation.
...
2. **Sync task/plan status (MANDATORY)** → use `TaskUpdate`/`TaskList` to sync plan/task status (if fix is part of a plan), update progress, hydrate Claude Tasks, generate status report
...
- `Explore` (Step 1) — understand before diagnosing

**Always activate (Step 6 Finalize):**
- Native Task tools (`TaskUpdate`/`TaskList`) — MANDATORY for sync-back and progress tracking, every fix
